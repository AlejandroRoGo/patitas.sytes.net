                ...
                <div class="list-group mb-4">
                    ...
                    <div class="list-group-item bg-white mb-3 rounded shadow-sm p-3 d-flex flex-column flex-md-row align-items-center gap-3 product-list-item">
                        ...
                        <div class="flex-grow-1 w-100">
                            ...
                            <div class="product-desc mb-2 d-none d-md-block"><?= htmlspecialchars(mb_strimwidth($prod['descripcion'],0,120,'...')) ?></div>
                            <div class="d-flex flex-wrap align-items-center gap-3 mb-2">
                                <span class="product-price">€<?= number_format($prod['precio'],2) ?></span>
                                <span class="badge bg-secondary">Stock: <?= $prod['stock'] ?></span>
                            </div>
                            <div class="d-flex gap-2 flex-wrap">
                                <form method="post" action="agregar_carrito.php" class="d-flex align-items-center gap-2">
                                    <input type="hidden" name="id_producto" value="<?= $prod['id_producto'] ?>">
                                    <input type="number" name="cantidad" value="1" min="1" max="<?= $prod['stock'] ?>" class="form-control form-control-sm" style="max-width:80px;">
                                    <button type="submit" class="btn btn-comprar btn-sm">Agregar al carrito</button>
                                </form>
                                <a href="producto.php?id=<?= $prod['id_producto'] ?>" class="btn btn-outline-secondary btn-sm">+ Detalles</a>
                            </div>
                        </div>
                    </div>
                    <?php endforeach; ?>
